...
namespace SqlEditor.SqlParser.Parsers
{
    ...
    public class ExpressionParser : CustomParser
    {
        ...
        private Expression ReadCriteria(Expression parent)
        {
            ...
            else if (Tokenizer.TokenEquals(Constants.Between))
                return ProcessBetween(parent, expression);
            else if (Tokenizer.IsNextToken("=", "<>", "!=", ">=", "<=", ">", "<", "IS", "IN", "ANY", "LIKE"))
        ...
        private Expression ReadFactor(Expression parent)
        {
            ...
            else
            {
                ...
                string token;
                if (Tokenizer.HasMoreTokens && Tokenizer.Current.Type == TokenType.SingleQuote)
                    // StartsWith( Constants.Quote ) && Tokenizer.Current.EndsWith( Constants.Quote ) )
                {
