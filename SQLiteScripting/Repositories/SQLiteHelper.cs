...
namespace ErikEJ.SQLiteScripting
{
    public class SQLiteHelper : ISqlCeHelper
    {
        ...
        public void SaveDataConnection(string repositoryConnectionString, string connectionString, int dbType)
        {
            ...
            {
                string filePath = System.IO.Path.GetFileName(PathFromConnectionString(connectionString));
        ...
        public string FormatError(Exception e)
        {
            if (e.GetType() == typeof(SQLiteException))
            {
                ...
                bld.AppendLine("Result    : " + Enum.GetName(typeof(SQLiteErrorCode), ex.ErrorCode));
                return bld.ToString();        
