...
namespace SqlEditor.Databases.PostgreSql
{
    public class PostgreSqlInfoProvider : DbInfoProvider
    {
        ...
        public override IList<Schema> GetSchemas(IDbConnection connection,
            [JetBrains.Annotations.NotNull] string databaseInstance = null)
        {
            if (connection == null) throw new ArgumentNullException("connection");
            if (databaseInstance == null) throw new ArgumentNullException("databaseInstance");
            return GetSchemasBase(connection, "SELECT schema_name FROM information_schema.schemata where UPPER(catalog_name) = @1 ORDER BY schema_name", databaseInstance.Trim().ToUpper());
        }
            ...
            [JetBrains.Annotations.NotNull] string databaseInstanceName = null)
        {
            ...
            return GetTablesBase(connection, schemaName, "SELECT table_name FROM information_schema.tables WHERE table_type = 'BASE TABLE' AND UPPER(table_schema) = @1 AND UPPER(table_catalog) = @2", schemaName.Trim().ToUpper(), databaseInstanceName.Trim().ToUpper());
        }

        public override IList<Column> GetTableColumns([NotNull] IDbConnection connection, [NotNull] string schemaName, [NotNull] string tableName,
            [JetBrains.Annotations.NotNull] string databaseInstanceName = null)
        {
            if (connection == null) throw new ArgumentNullException("connection");
            ...
            if (databaseInstanceName == null) throw new ArgumentNullException("databaseInstanceName");
            return GetTableColumnsBase(connection, schemaName, tableName, "SELECT column_name, data_type, character_maximum_length, numeric_precision, numeric_scale, is_nullable, ordinal_position FROM information_schema.columns WHERE UPPER(table_schema) = @1 AND UPPER(table_name) = @2 AND UPPER(table_catalog) = @3 ORDER BY ordinal_position", schemaName.Trim().ToUpper(), tableName.Trim().ToUpper(), databaseInstanceName.Trim().ToUpper());
        }
            ...
            [JetBrains.Annotations.NotNull] string databaseInstanceName = null)
        {
            ...
            if (tableName == null) throw new ArgumentNullException("tableName");
            if (databaseInstanceName == null) throw new ArgumentNullException("databaseInstanceName");
            return GetTableColumnsBase(connection, schemaName, tableName, "SELECT c.column_name, c.data_type, c.character_maximum_length, c.numeric_precision, c.numeric_scale, c.is_nullable, c.ordinal_position   FROM information_schema.columns c INNER JOIN information_schema.key_column_usage k 	USING(table_schema,table_name,column_name) INNER JOIN information_schema.table_constraints t USING(table_schema,table_name,constraint_name) WHERE UPPER(c.table_schema) = @1 AND UPPER(c.table_name) = @2   AND UPPER(c.table_catalog) = @3  AND t.constraint_type='PRIMARY KEY'", schemaName.Trim().ToUpper(), tableName.Trim().ToUpper(), databaseInstanceName.Trim().ToUpper());
        }
            ...
            [JetBrains.Annotations.NotNull] string databaseInstanceName = null)
        {
            ...
            return GetViewsBase(connection, schemaName, "SELECT table_name FROM information_schema.views WHERE UPPER(table_schema) = @1 AND UPPER(table_catalog) = @2", schemaName.Trim().ToUpper(), databaseInstanceName.Trim().ToUpper());
        }
            ...
            [JetBrains.Annotations.NotNull] string databaseInstanceName = null)
        {
            ...
            if (databaseInstanceName == null) throw new ArgumentNullException("databaseInstanceName");
            return GetTableColumnsBase(connection, schemaName, viewName, "SELECT column_name, data_type, character_maximum_length, numeric_precision, numeric_scale, is_nullable, ordinal_position FROM information_schema.columns WHERE UPPER(table_schema) = @1 AND UPPER(table_name) = @2  AND UPPER(table_catalog) = @3 ORDER BY ordinal_position", schemaName.Trim().ToUpper(), viewName.Trim().ToUpper(), databaseInstanceName.Trim().ToUpper());
        }
        ...
        public override IList<Index> GetIndexes([NotNull] IDbConnection connection, [NotNull] string schemaName, string databaseInstanceName = null)
        {
            ...
            return GetIndexesBase(connection, schemaName,
                                  "SELECT null as idx_id, ns.nspname as table_schema, idx.indrelid::regclass as table_name, idx.indrelid::regclass as index_schema, i.relname as index_name, CASE WHEN indisunique = 't' THEN 1 ELSE 0 END as is_unique FROM pg_index as idx JOIN pg_class as i ON i.oid = idx.indexrelid JOIN pg_am as am ON i.relam = am.oid JOIN pg_namespace as ns ON ns.oid = i.relnamespace AND UPPER(ns.nspname) = @1 ORDER BY i.relname",
