...
namespace SqlEditor.Intellisense
{
    public class CodeCompletionWindow : AbstractCompletionWindow
    {
        ...
        {
            return ShowCompletionWindow(parent, control, fileName, completionDataProvider, firstChar, true, true, -1,-1);
        }
        ...
        {
            Debug.WriteLine("StartOffset (document about to be changed): {0}.", startOffset);
        ...
        protected override void CaretOffsetChanged(object sender, EventArgs e)
        {
            ...
            {
                //var text = control.Document.GetText(startOffset, offset - startOffset).ToUpper();
                //codeCompletionListView.SelectItemWithStart(text);
            }
        }
        ...
        bool InsertSelectedItem(char ch)
        {
            ...
            if (data != null)
            {
                ...
                try
                {
                    ...
                    {
                        throw new Exception("Program error. Start offset: " + startOffset + " is greater than document length: " + document.TextLength);
                    }
        ...
        int GetListViewWidth(int defaultWidth, int height)
        {
            ...
            using (Graphics graphics = codeCompletionListView.CreateGraphics())
            {
                ...
                {
                    var itemWidth = graphics.MeasureString(completionData[i].Text.ToString(), codeCompletionListView.Font).Width;
                    if (itemWidth > width)
                    {
                        width = itemWidth;
                    }
